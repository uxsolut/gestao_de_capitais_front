...
import '../../widgets/modals/robos_da_conta_dialog.dart';
import '../../widgets/topo_fixo.dart';

class ClienteOrdensPage extends StatefulWidget {
final int idRoboUser;
final int? voltarParaConta;
final String? voltarParaContaNome;

const ClienteOrdensPage({
  super.key,
  required this.idRoboUser,
  this.voltarParaConta,
  this.voltarParaContaNome,
});
  @override
  State<ClienteOrdensPage> createState() => _ClienteOrdensPageState();
}

class _ClienteOrdensPageState extends State<ClienteOrdensPage> with TickerProviderStateMixin {
  ...
  Future<void> _loadOrdens() async {
    ...
    try {
      final ordens = await _ordemService.getOrdensByRoboUser(widget.idRoboUser);
    ...
    children: [
      Row(
        children: [
          IconButton(
            onPressed: () {
              if (widget.voltarParaConta != null && widget.voltarParaContaNome != null) {
                Navigator.pop(context);
                Future.delayed(const Duration(milliseconds: 100), () {
                  showDialog(
                    context: context,
                    builder: (_) => RobosDaContaDialog(
                      idConta: widget.voltarParaConta!,
                      nomeConta: widget.voltarParaContaNome!,
                    ),
                  );
                });
              } else {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
              'Ative seu robô e espere até as ordens começarem a serem feitas',
              style: TextStyle(
                color: Colors.white70,
                fontSize: 14,
              ),
      ...
      child: Column(
        ...
        children: [
          ...
          if (ordem.idUser != null) ...[
            _buildDetailRow('Usuário', 'User ${ordem.idUser}', Icons.person),
            const SizedBox(height: 8),
          ],
